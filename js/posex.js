...
function create_body(unit, x0, y0, z0) {
    ...
    for (let i = 0; i < standard_pose.length; ++i) {
        ...
        const geom = new THREE.SphereGeometry(JOINT_RADIUS, 128, 128);
...
function init_3d(ui) {
    ...
    const indicator1 = ui.indicator1;
    const indicator2 = ui.indicator2;
...
renderer.domElement.addEventListener('pointerdown', e => {
    ...
    const width_input = ui.canvas_width;
    const height_input = ui.canvas_width;
    if (width_input && height_input) {
        width_input.addEventListener('change', () => {
            let res = width_input.value;
            res = res.split(" ");
